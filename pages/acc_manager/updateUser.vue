			...
			<uni-forms ref="form" class="form" :model="FormData" :rules="rules" label-position="top"
				...
				<uni-forms-item label="用户名" name="username" :required="true">
					<view class="easyInput">
						<uni-easyinput confirmType='next' :clearable="true" class="easyInput" v-model="FormData.username"
							type="easyInput" />
					</view>
				</uni-forms-item>
				...
				<uni-forms-item label="密码" name="password" :required="true">
					<view class="easyInput">
						<uni-easyinput confirmType='next' :clearable="true" v-model="FormData.password" type="password" />
					</view>
				</uni-forms-item>
...
<script>
	export default {
		...
		onReady() {
			if (this.id != 0) {
				uni.request({
					url: this.$http + "administration/getUserById?id=" + this.id,
				...
				if (!this.locked) {
					...
					this.$refs.form.validate([], (err, FormData) => {
						if (!err) {
								...
								uni.request({
									url: this.$http + "administration/updateUser",
									method: "POST",
									data: this.FormData,
									header: {
										"token": token
									...
									success: (res) => {
										...
										setTimeout(() => {
											this.locked = false
											uni.reLaunch({
												url: '/pages/acc_manager/acc_manager'
											})
										}, 1000)
									}
								})
